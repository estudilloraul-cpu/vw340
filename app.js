...
function openDay(id){const d=D.trip.find(x=>x.id===id),done=store.get('doneDays',[]),journal=store.get('journal',{});if(!d)return;const idx=D.trip.findIndex(x=>x.id===id),prev=D.trip[idx-1],next=D.trip[idx+1];const dayNav=`<nav class="day-nav" aria-label="Navegación entre jornadas">${prev?`<button class="day-nav-btn prev" data-day="${prev.id}"><small>‹ DÍA ANTERIOR</small><b>${esc(prev.city)}</b></button>`:'<span></span>'}${next?`<button class="day-nav-btn next" data-day="${next.id}"><small>SIGUIENTE DÍA ›</small><b>${esc(next.city)}</b></button>`:'<span></span>'}</nav>`;openModal(`<div class="detail-hero"><img src="${d.photo}" alt=""><div class="detail-title"><div class="eyebrow">${fmtDate(d.date)} · ${esc(d.city)}</div><h1>${esc(d.title)}</h1></div></div><div class="detail-content">${dayNav}<p>${esc(d.meta)}</p><h3>Agenda</h3><div class="panel agenda-panel">${d.items.map(x=>`<p>${esc(x)}</p>`).join('')}</div>${transportCard(d.transport)}${overnightCard(OVERNIGHTS[d.id])}${diningCard(d.dining)}${extrasCard(DAY_EXTRAS[d.id])}${d.places.length?`<h3>Lugares del día</h3><div class="grid">${d.places.map(id=>placeCard(D.places.find(p=>p.id===id))).join('')}</div>`:''}<div class="actions"><button class="btn" data-toggle-day="${d.id}">${done.includes(d.id)?'Marcar como pendiente':'Completar jornada'}</button><button class="btn ghost" data-journal-day="${d.id}">Escribir recuerdo</button></div>${journal[d.id]?`<div class="panel">${journal[d.id].photo?`<img class="memory-photo" src="${journal[d.id].photo}" alt="Foto del recuerdo">`:''}<b>Tu recuerdo</b><p>${esc(journal[d.id].text)}</p><p>${'★'.repeat(Number(journal[d.id].rating||0))}</p></div>`:''}${dayNav}</div>`) }
