...
function home(){
 ...
 let idx=D.trip.findIndex(d=>new Date(d.date+'T12:00:00')>=now);
 if(idx<0)idx=D.trip.length-1;
 const today=D.trip[idx]||D.trip[0],next=D.trip[Math.min(idx+1,D.trip.length-1)]||today;
 const lived=D.trip.filter(d=>new Date(d.date+'T23:59:59')<now).length;
 return hero(today.photo,`${fmtDate(today.date)} · ${today.city}`,today.title,today.meta,`<button class="btn secondary" data-day="${today.id}">Abrir jornada ›</button>`)+
 `<section class="section"><div class="section-head"><div><h2>El viaje real</h2><p>La guía se va quedando con lo que de verdad nos funcionó.</p></div></div><div class="travel-stats"><div class="travel-stat"><i><svg viewBox='0 0 24 24'><path d='M12 21s6-5.2 6-11a6 6 0 1 0-12 0c0 5.8 6 11 6 11Z'/><circle cx='12' cy='10' r='2.2'/></svg></i><b>${lived} días</b><span>Ya vividos</span></div><div class="travel-stat"><i><svg viewBox='0 0 24 24'><rect x='4' y='5.5' width='16' height='14' rx='2'/><path d='M8 3v5M16 3v5M4 10h16'/></svg></i><b>11–20 ago</b><span>Portugal + vuelta</span></div><div class="travel-stat"><i><svg viewBox='0 0 24 24'><path d='M4 20V9l5 3V5l6 4v11M15 12l5-3v11M7 16h1M11 15h1M17 15h1'/></svg></i><b>${D.cities.length} zonas</b><span>Ruta viva</span></div><div class="travel-stat"><i><svg viewBox='0 0 24 24'><circle cx='12' cy='12' r='9'/><path d='m12 7 1.5 3 3.5.5-2.5 2.5.6 3.5-3.1-1.7-3.1 1.7.6-3.5L7 10.5l3.5-.5z'/></svg></i><b>${Object.values(missionLists()).reduce((n,a)=>n+a.length,0)}</b><span>Misiones</span></div></div></section>
 <section class="section"><div class="section-head"><div><h2>Lo siguiente</h2></div></div><article class="next-card"><img src="${next.photo}" alt="${esc(next.city)}"><button data-day="${next.id}"><small>${fmtDate(next.date)}</small><h3>${esc(next.city)}</h3><p>${esc(next.title)}</p><span class="arrow">›</span></button></article></section>
 <section class="section"><div class="section-head"><div><h2>Últimas jornadas</h2><p>El planning original deja paso a lo que realmente pasó.</p></div><button class="linkbtn" data-go="route">Ver ruta</button></div><div class="timeline">${D.trip.slice(Math.max(0,idx-2),Math.min(D.trip.length,idx+1)).map(dayRow).join('')}</div></section>
 <section class="section"><div class="section-head"><div><h2>Esto sí nos representa</h2><p>Lugares que encajan con la nueva forma de viajar.</p></div><button class="linkbtn" data-go="explore">Explorar</button></div><div class="grid">${['lxfactory','aveiro-centro','gazela','bombarda'].map(id=>placeCard(D.places.find(p=>p.id===id))).join('')}</div></section>`
}
function routePage(){return hero('images/familia-vw340.jpg','11–20 agosto','Portugal · la ruta que ocurrió','De Lisboa a Oporto cambiamos el plan sobre la marcha: menos lista de imprescindibles y más lugares donde nos apetecía estar.',`<a class="btn secondary" target="_blank" rel="noopener" href="https://www.google.com/maps/dir/Chiclana+de+la+Frontera/Lisboa/%C3%93bidos/Nazar%C3%A9/Vieira+de+Leiria/Praia+de+Mira/Costa+Nova/Aveiro/Espinho/Porto/Vila+Nova+de+Gaia/Peso+da+Regua/Tobera/Frias/Logrono/Sabadell">Abrir ruta real en Maps ›</a>`)+`<section class="section"><div class="section-head"><div><h2>Jornadas</h2><p>Cada ficha mezcla lo vivido, lo que repetiríamos y lo siguiente.</p></div></div><div class="timeline route-line">${D.trip.map(dayRow).join('')}</div></section><section class="section"><div class="section-head"><div><h2>Destinos</h2><p>Ya no son una lista turística: son nuestra lectura de cada lugar.</p></div></div><div class="city-grid">${D.cities.map(c=>`<article class="city-cover"><img src="${c.photo}" alt="${esc(c.name)}"><button data-city="${esc(c.name)}"><small>${c.count} lugares</small><h3>${esc(c.name)}</h3><p>${esc(c.subtitle)}</p></button></article>`).join('')}</div></section>`}
...
function openDay(id){const d=D.trip.find(x=>x.id===id),done=store.get('doneDays',[]),journal=store.get('journal',{});if(!d)return;const idx=D.trip.findIndex(x=>x.id===id),prev=D.trip[idx-1],next=D.trip[idx+1];const dayNav=`<nav class="day-nav" aria-label="Navegación entre jornadas">${prev?`<button class="day-nav-btn prev" data-day="${prev.id}"><small>‹ DÍA ANTERIOR</small><b>${esc(prev.city)}</b></button>`:'<span></span>'}${next?`<button class="day-nav-btn next" data-day="${next.id}"><small>SIGUIENTE DÍA ›</small><b>${esc(next.city)}</b></button>`:'<span></span>'}</nav>`;openModal(`<div class="detail-hero"><img src="${d.photo}" alt=""><div class="detail-title"><div class="eyebrow">${fmtDate(d.date)} · ${esc(d.city)}</div><h1>${esc(d.title)}</h1></div></div><div class="detail-content">${dayNav}<p>${esc(d.meta)}</p><h3>Agenda</h3><div class="panel agenda-panel">${d.items.map(x=>`<p>${esc(x)}</p>`).join('')}</div>${d.story?`<section class="panel real-story"><small class="section-kicker">LO QUE PASÓ</small><h3>La jornada real</h3><p>${esc(d.story)}</p>${d.verdict?`<div class="story-note moment-note"><small>NUESTRA LECTURA</small><h4>${esc(d.verdict)}</h4></div>`:''}</section>`:''}${transportCard(d.transport)}${overnightCard(OVERNIGHTS[d.id])}${diningCard(d.dining)}${extrasCard(DAY_EXTRAS[d.id])}${d.places.length?`<h3>Lugares del día</h3><div class="grid">${d.places.map(id=>placeCard(D.places.find(p=>p.id===id))).join('')}</div>`:''}<div class="actions"><button class="btn" data-toggle-day="${d.id}">${done.includes(d.id)?'Marcar como pendiente':'Completar jornada'}</button><button class="btn ghost" data-journal-day="${d.id}">Escribir recuerdo</button></div>${journal[d.id]?`<div class="panel">${journal[d.id].photo?`<img class="memory-photo" src="${journal[d.id].photo}" alt="Foto del recuerdo">`:''}<b>Tu recuerdo</b><p>${esc(journal[d.id].text)}</p><p>${'★'.repeat(Number(journal[d.id].rating||0))}</p></div>`:''}${dayNav}</div>`) }
