...
function detailedDayCard(id){if(id!=='D1')return '';return `<section class="detail-day-entry"><div><small>RUTA PASO A PASO</small><h3>Día al detalle</h3><p>Recorre Lisboa tramo a tramo, con calles, curiosidades y navegación entre cada parada.</p></div><button class="btn detail-day-open" data-detail-step="0">Abrir recorrido ›</button></section>`}
function openDetailedDay(index=0){const i=Math.max(0,Math.min(Number(index)||0,LISBON_DETAIL.length-1)),s=LISBON_DETAIL[i],prev=LISBON_DETAIL[i-1],next=LISBON_DETAIL[i+1];openModal(`<div class="detail-step-hero"><img src="${s.image}" alt="${esc(s.title)}"><div class="detail-step-overlay"><small>DÍA 12 · ${i+1} DE ${LISBON_DETAIL.length}</small><div class="detail-step-time">${esc(s.time)}</div><h1>${esc(s.title)}</h1></div></div><div class="detail-content detailed-day"><div class="detail-progress"><span style="width:${((i+1)/LISBON_DETAIL.length)*100}%"></span></div><div class="detail-route"><small>POR DÓNDE IR</small><b>${esc(s.route)}</b></div><p class="detail-step-copy">${esc(s.text)}</p><h3>Fijaos en esto</h3><div class="detail-look-list">${s.look.map(x=>`<span>✓ ${esc(x)}</span>`).join('')}</div><nav class="detail-step-nav">${prev?`<button data-detail-step="${i-1}" class="detail-step-btn prev"><small>‹ ANTERIOR</small><b>${esc(prev.time)}</b><span>${esc(prev.title)}</span></button>`:'<span></span>'}${next?`<button data-detail-step="${i+1}" class="detail-step-btn next"><small>SIGUIENTE ›</small><b>${esc(next.time)}</b><span>${esc(next.title)}</span></button>`:`<button data-day="D1" class="detail-step-btn next"><small>FIN DEL RECORRIDO</small><b>Volver al día</b><span>Ver agenda completa</span></button>`}</nav><button class="btn ghost detail-back-day" data-day="D1">← Volver a la jornada</button></div>`)}
